...
import { getOrganisationDetails } from "@/lib/api/chat";
        ...
        <div className="flex-1 overflow-y-auto max-h-[500px] p-4 space-y-3">
          ...
          {organisations.map((udruga) => (
            <div
              ...
              onClick={async () => {
                try {
                  const data = await getOrganisationDetails(udruga.name);
                  const parts = [];
                  if (data?.mission) parts.push(data.mission);
                  const listify = (arr) => (Array.isArray(arr) && arr.length ? `• ${arr.join("\n• ")}` : "");
                  const sections = [
                    ["Djelokrug", listify(data?.domains)],
                    ["Aktivnosti", listify(data?.core_activities)],
                    ["Projekti", listify(data?.flagship_projects)],
                    ["Ciljna skupina", data?.target_members],
                    ["Vještine/ishodi", listify(data?.skills_outcomes)],
                    ["Ključne riječi", listify(data?.keywords)],
                    ["Poveznice", listify(data?.affiliations)],
                  ];
              ...
              <div className="flex items-center justify-between mb-1">
                {udruga.url ? (
                  <a
                    ...
                    className="font-semibold text-gray-900 dark:text-white text-sm group-hover:text-green-700 dark:group-hover:text-green-400 transition-colors underline decoration-transparent hover:decoration-green-400"
                  >
                    {udruga.name}
                  </a>
                ) : (
    ...
    {modalOpen && (
      ...
      >
        ...
        <div className="relative mx-4 w-full max-w-2xl rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-2xl p-6 max-h-[80vh] overflow-auto">
          <div className="mb-4">
            <h3 id="org-modal-title" className="text-lg font-semibold text-gray-900 dark:text-white">
              {modalData?.name}
            </h3>
